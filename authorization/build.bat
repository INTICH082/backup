...
if %errorlevel% equ 0 (echo ✅ libcurl-4.dll) else (echo ❌ libcurl-4.dll not found)

copy "C:\msys64\ucrt64\bin\libssl-3-x64.dll" build\ >nul 2>&1
if %errorlevel% equ 0 (echo ✅ libssl-3-x64.dll) else (echo ❌ libssl-3-x64.dll not found)

copy "C:\msys64\ucrt64\bin\libcrypto-3-x64.dll" build\ >nul 2>&1
if %errorlevel% equ 0 (echo ✅ libcrypto-3-x64.dll) else (echo ❌ libcrypto-3-x64.dll not found)

copy "C:\msys64\ucrt64\bin\zlib1.dll" build\ >nul 2>&1
if %errorlevel% equ 0 (echo ✅ zlib1.dll) else (echo ❌ zlib1.dll not found)
